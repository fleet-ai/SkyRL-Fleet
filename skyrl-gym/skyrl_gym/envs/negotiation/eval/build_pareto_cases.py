...
def main():
    ...
    for case_no, fkey, fpretty, s in CASES:
        ...
        if any(x is None for x in (fd, qd, fs, qs)):
            print(
                f"!! case {case_no}: missing (f-dual={fd is not None}, q-dual={qd is not None}, "
                f"f-single={fs is not None}, q-single={qs is not None})"
            )
            continue
        fdg, qdg, fsg, qsg = (E.to_viz_game(fd), E.to_viz_game(qd), E.to_viz_game(fs), E.to_viz_game(qs))
    ...
    (OUT / f"{runid}.json").write_text(
        json.dumps({"split": "all", "dataset": "eval", "model": "Pareto case studies", "stats": stats, "games": games})
    )
    ...
    man["datasets"].insert(
        0,
        {
            ...
            "blurb": (
                "Same dnd scenarios, four cards each in matched same-protocol pairs: frontier vs "
