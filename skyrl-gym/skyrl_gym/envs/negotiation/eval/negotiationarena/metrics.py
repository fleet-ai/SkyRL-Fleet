...
def _metric_block(results: list[GameResult]) -> dict:
    ...
    if n_decisive > 0:
        focal_win_rate = _mean(
            [1.0 if r.focal_win else 0.0 for r in decisive_results]
        )
    else:
        focal_win_rate = None
    ...
    format_violation_rate = _mean(
        [1.0 if r.format_violation else 0.0 for r in results]
    )
    ...
    if has_sell_buy:
        agreed_sb = [
            r for r in results if r.scenario.game == "sell_buy" and r.deal
        ]
    ...
    if has_ultimatum:
        agreed_ult = [
            r for r in results if r.scenario.game == "ultimatum" and r.deal
        ]
...
def compute_metrics(
    ...
    by_focal_role: dict[str, dict] = {
        role: _metric_block([r for r in results if r.scenario.focal_role == role])
...
if __name__ == "__main__":
    ...
    re3 = _gr(sc_re3, deal=False, focal_payoff=0.0,  opp_payoff=0.0,  n_turns=8,
              format_violation=True, error="timeout")
    ...
    ult1 = _gr(sc_ult1, deal=True,  focal_payoff=70.0, opp_payoff=30.0,
               n_turns=2, proposer_give=30.0)
    # focal is responder (seat 1), got 40 from proposer who kept 60
    ult2 = _gr(sc_ult2, deal=True,  focal_payoff=40.0, opp_payoff=60.0,
               n_turns=2, proposer_give=40.0)
    ult3 = _gr(sc_ult3, deal=False, focal_payoff=0.0,  opp_payoff=0.0,  n_turns=8)

    # sell_buy: 4 agreed (exercises anchoring ≥ 3) + 1 no-deal
    sc_sb1 = _scenario("sell_buy", focal_seat=0, episode_id="sb-0",
                        seller_cost=40, buyer_willingness=60)
    sc_sb2 = _scenario("sell_buy", focal_seat=0, episode_id="sb-1",
                        seller_cost=30, buyer_willingness=70)
    sc_sb3 = _scenario("sell_buy", focal_seat=1, episode_id="sb-2",
                        seller_cost=20, buyer_willingness=80)
    sc_sb4 = _scenario("sell_buy", focal_seat=1, episode_id="sb-3",
                        seller_cost=35, buyer_willingness=65)
    sc_sb5 = _scenario("sell_buy", focal_seat=0, episode_id="sb-4",
                        seller_cost=40, buyer_willingness=60)
    sb1 = _gr(sc_sb1, deal=True,  focal_payoff=10.0, opp_payoff=10.0,
              n_turns=3, sale_price=50.0, focal_opening_price=70.0)
    sb2 = _gr(sc_sb2, deal=True,  focal_payoff=15.0, opp_payoff=20.0,
              n_turns=4, sale_price=45.0, focal_opening_price=65.0)
    sb3 = _gr(sc_sb3, deal=True,  focal_payoff=25.0, opp_payoff=15.0,
              n_turns=5, sale_price=55.0, focal_opening_price=45.0)
    sb4 = _gr(sc_sb4, deal=True,  focal_payoff=20.0, opp_payoff=10.0,
              n_turns=4, sale_price=55.0, focal_opening_price=40.0)
    ...
    assert overall["anchoring_spearman"] is not None, (
    ...
    assert list(out["by_game"].keys()) == ["resource_exchange", "ultimatum", "sell_buy"], (
        out["by_game"].keys()
    )

    # by_focal_role keys span all roles encountered.
    for expected_role in ("player_1", "player_2", "proposer", "responder", "seller", "buyer"):
        assert expected_role in out["by_focal_role"], (
            f"Missing role '{expected_role}' in by_focal_role"
        )
    ...
    assert out2["overall"]["anchoring_spearman"] is None, (
    ...
    assert out_ties["overall"]["focal_win_rate"] is None, (
        "Expected None focal_win_rate on all-tie slice"
    )
