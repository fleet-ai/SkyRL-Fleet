    ...
    async def init_async(
        ...
        if self.enable_meta_tools and self.tool_index:
            ...
            tools_section = (
                ...
                f"Example workflow:\n"
                f"  Task: 'Find emails with attachments and check the attachment names'\n"
                f"  Step 1: search_tools('attachment')  → compare results:\n"
                f"    - list_email_attachments: 'List all attachments for an email message'\n"
                f"    - get_email_attachment: 'Download attachment content from an email...'\n"
                f"  Step 2: Pick list_email_attachments (need names, not file content)\n"
                f"  Step 3: get_tool_schema('list_email_attachments')  → see exact parameters\n"
                f"  Step 4: Call list_email_attachments with correct arguments\n\n"
