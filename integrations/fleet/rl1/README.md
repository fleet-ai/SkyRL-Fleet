...
This starts a build job on the cluster's builder machine. It clones the branch from GitHub (so uncommitted local changes never end up in an image), builds the Dockerfile in this directory, and pushes the result to GitHub Container Registry: `ghcr.io/fleet-ai/skyrl-fleet/trainer`. The tag is the 8-character commit sha of what was built, so any image can be traced back to its source; `:latest` moves with every build. Pushing uses the `img-build-secrets` token already on the cluster; pods pull with the `ghcr-pull` secret. Neither needs setup for rl1. The image contains everything the setup scripts would install on a fresh VM: the locked Python dependencies, transformers 5.3.0, the flash-attn wheel, causal-conv1d compiled for both our GPU generations (sm_90 and sm_100, so one image runs on either pool), OpenEnv pinned to a specific commit, the CUDA 12.8 compiler (Qwen3.5 compiles some GPU kernels at training time), and `ray[default]` (KubeRay's health checks need it; without it jobs cannot start). The build fails unless the training code imports cleanly.
