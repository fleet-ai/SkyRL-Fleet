#!/usr/bin/env bash
# Launch taste-add: taste judge enabled (claude-haiku-4-5 via openrouter, floor=0.1).
#
# Usage:
#   export FLEET_API_KEY=... WANDB_API_KEY=... AWS_ACCESS_KEY_ID=...
#   export AWS_SECRET_ACCESS_KEY=... OPENROUTER_API_KEY=... ANTHROPIC_API_KEY=... HF_TOKEN=...
#   bash scripts/fleet-taste-judge-launch.sh
set -eo pipefail
...
CLUSTER="taste-add"
RUN_NAME="fleet_qwen35_browser_use_add-judge"

sky launch -c "$CLUSTER" --retry-until-up -y "$YAML" \
  --env RUN_ID="add-judge" \
  ...
  --env SKYRL_TASTE_MODEL=anthropic/claude-haiku-4-5 \
  --env SKYRL_TASTE_PROVIDER=openrouter \
