#!/usr/bin/env bash
# Single source of truth for Qwen3.5-9B GRPO tool_use training config.
# Called by the SkyPilot YAML, by fleet-research run.sh, and by the rl1 job
# examples (integrations/fleet/rl1/).
#
# Single-node config: TP=1, 8 engines on 8 GPUs. Model flags follow the two
# proven Qwen3.5-9B configs in this repo (fleet-vl-run.sh, fleet-task-gen-run.sh):
# flash_attn=false, no sample packing, loss_chunk_size 4096, TP=1. Batch and
# environment flags follow fleet-35b-run.sh (the production tool_use config).
#
...
bash scripts/fleet-common-run.sh \
  ...
  trainer.run_name="fleet_qwen35_9b_${MODALITY}_${RUN_ID:-$(head -c 4 /dev/urandom | xxd -p)}" \
  trainer.resume_mode=latest \
  trainer.ckpt_path="$HOME/ckpts/fleet_qwen35_9b_${MODALITY}" \
