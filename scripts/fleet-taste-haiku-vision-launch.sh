#!/usr/bin/env bash
# Launch taste-haiku-vision: Haiku judge with screenshot support (direct Anthropic SDK).
#
# Provider: haiku_vision — calls score_trajectory_haiku() which sends up to 4
# evenly-sampled JPEG screenshots to claude-haiku-4-5-20251001 alongside the
# action trace.  This gives the judge real ui_grounding signal at ~20x lower
# cost than Sonnet.
#
# Usage:
#   export FLEET_API_KEY=... WANDB_API_KEY=... AWS_ACCESS_KEY_ID=...
#   export AWS_SECRET_ACCESS_KEY=... ANTHROPIC_API_KEY=... HF_TOKEN=...
#   bash scripts/fleet-taste-haiku-vision-launch.sh
set -eo pipefail
...
CLUSTER="taste-haiku-vision"
RUN_NAME="fleet_qwen35_browser_use_haiku-vision-judge"

sky launch -c "$CLUSTER" --retry-until-up -y "$YAML" \
  --env RUN_ID="haiku-vision-judge" \
