#!/usr/bin/env bash
# Ablation C — Actions + screenshots, no reasoning traces.
# Haiku judge receives: TASK + ACTIONS + up to 4 screenshots.
# Reasoning traces explicitly blinded (SKYRL_TASTE_BLIND_REASONING=1) so this
# is a clean baseline for the reasoning ablation (compare with ablation-reasoning).
#
# Usage:
#   export FLEET_API_KEY=... WANDB_API_KEY=... AWS_ACCESS_KEY_ID=...
#   export AWS_SECRET_ACCESS_KEY=... ANTHROPIC_API_KEY=... HF_TOKEN=...
#   bash scripts/fleet-taste-ablation-both-launch.sh
set -eo pipefail
...
CLUSTER="taste-ablation-both"
RUN_NAME="fleet_qwen35_browser_use_ablation-both"

sky launch -c "$CLUSTER" --retry-until-up -y "$YAML" \
  --env RUN_ID="ablation-both" \
  ...
  --env SKYRL_TASTE_BLIND_REASONING=1 \
  --env TASTE_FLOOR=0.1 \
