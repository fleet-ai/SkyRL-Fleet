...
class SkyRLTrainConfig(BaseConfig):
    ...
    def from_cli_overrides(cls, args: Union[List[str], dict]) -> "SkyRLTrainConfig":
        ...
        # Defaults come from the dataclass field defaults (the single source of truth):
        # build_nested_dataclass fills every field not present in the overrides. The legacy
        # base YAMLs (ppo_base_config.yaml, skyrl_gym_config/default.yaml) were removed
        # upstream (#1684) once defaults moved into the dataclasses. We still translate the
        # legacy flat keys our run scripts use (e.g. generator.backend ->
        # generator.inference_engine.backend) before constructing the typed config.
        ...
        merged_dict = OmegaConf.to_container(overrides, resolve=True) or {}
