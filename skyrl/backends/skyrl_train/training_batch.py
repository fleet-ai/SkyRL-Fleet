...
def pad_training_input_batch(unpadded_batch: TrainingInputBatch, pad_size: int) -> TrainingInputBatch:
    ...
    for key, value in old_metadata.items():
        if key == "uids":
            ...
            # Fleet hint augmentation: padded rows are never hinted. Extend (not just deepcopy)
            # so the list length matches the padded batch — compute_advantages_and_returns indexes
            # is_hinted by the padded is_last_step mask.
            new_metadata["is_hinted"] = value + [False for _ in range(pad_size)]
        else:
